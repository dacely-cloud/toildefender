...
var UniqueRandom = class {
	...
	max;
	constructor(max) {
		(0, assert.default)(typeof max == "number");
		if (max > 32768) console.warn(`Allocating large (${max}) UniqueRandom instance`);
		this.max = max;
		this.arr = lodash.default.shuffle(lodash.default.range(max));
	}
	get() {
...
var UniqueRandomAlpha = class {
	...
	rng;
	constructor(len) {
		(0, assert.default)(typeof len == "number");
...
var VISITOR_KEYS = Object.assign({}, estraverse.default.VisitorKeys, {
...
function traverse(node, stack, processor) {
	...
	assert.default.ok(Array.isArray(stack));
...
var ESUtils = class {
	...
	replaceNode(root, child, replacement) {
		assert.default.ok(estest_default.isNode(root));
		assert.default.ok(estest_default.isNode(child));
		assert.default.ok(estest_default.isNode(replacement));
		assert.default.equal(estest_default.isStatement(child), estest_default.isStatement(replacement), `Replacee ${child.type} is not of the same type as replacement ${replacement.type}`);
		assert.default.equal(estest_default.isExpression(child), estest_default.isExpression(replacement), `Replacee ${child.type} is not of the same type as replacement ${replacement.type}`);
...
function createMethodStub(id) {
	assert.default.equal(id.type, "Identifier");
	return {
		type: "CallExpression",
		callee: {
			type: "Identifier",
			...
			scope.variables.forEach((variable) => {
				variable.defs.forEach((def) => {
					if (def.type == "FunctionName") {
						(0, assert.default)(estest_default.isFunction(def.node));
						/**
...
var Flattener = class {
	...
	*/
	addMethod(input, entry, exit) {
		assert.default.ok(estest_default.isStatement(input));
	...
	*/
	transformStatement(node, entry, exit) {
		(0, assert.default)(estest_default.isStatement(node));
		assert.default.equal(typeof entry, "number");
		assert.default.equal(typeof exit, "number");
		switch (node.type) {
			case "Program":
	...
	*/
	transformSequence(node, entry, exit) {
		assert.default.equal(node.type, "BlockStatement");
		assert.default.equal(typeof entry, "number");
		assert.default.equal(typeof exit, "number");
		const stmts = [];
	...
	*/
	transformIf(node, entry, exit) {
		assert.default.equal(node.type, "IfStatement");
		assert.default.equal(typeof entry, "number");
		assert.default.equal(typeof exit, "number");
	...
	*/
	transformWhile(node, entry, exit) {
		assert.default.equal(node.type, "WhileStatement");
		assert.default.equal(typeof entry, "number");
		assert.default.equal(typeof exit, "number");
	...
	*/
	transformDoWhile(node, entry, exit) {
		assert.default.equal(node.type, "DoWhileStatement");
		assert.default.equal(typeof entry, "number");
		assert.default.equal(typeof exit, "number");
	...
	*/
	transformSwitch(node, entry, exit) {
		assert.default.equal(node.type, "SwitchStatement");
		assert.default.equal(typeof entry, "number");
		assert.default.equal(typeof exit, "number");
		const comps = [];
		this.breaks.push({
...
function chain(expressions, operator) {
	assert.default.ok(Array.isArray(expressions));
	assert.default.equal(typeof operator, "string");
...
function modernMangle(ast) {
	const scopeManager = escope.default.analyze(ast, {
		ecmaVersion: 6,
		optimistic: true,
		sourceType: "script"
	});
...
var Uglifier = class {
	...
	*/
	uglify(ast) {
		...
		return esshorten.default.mangle(ast);
	}
};
//#endregion
...
function makeStringGenerator(str) {
	assert.default.equal(typeof str, "string");
	const fragments = [];
	while (str.length > 0) {
...
function hashSeed(seed) {
	return crypto.default.createHash("sha256").update(String(seed)).digest().readUInt32LE(0) || 1;
}
...
function requireBase() {
	...
	return node_path.default.join(process.cwd(), "toildefender.js");
}
var optionalRequire = (0, node_module.createRequire)(requireBase());
	...
	*/
	...
	function parseSource(code, options) {
		try {
			...
			if (!_babel_parser) throw esprimaError;
			const parsed = _babel_parser.parse(code, {
				sourceType: "unambiguous",
				plugins: [
					"estree",
					"classProperties",
	...
	doTask("scope", options.features.scope, () => {
		...
		doTask("obfuscate_identifiers", true, () => {
			...
			variables.functionDeclarationToExpression(ast, escope.default.analyze(ast, scopeOptions));
			variables.obfuscateIdentifiers(ast, escope.default.analyze(ast, lexicalScopeOptions));
			variables.redefineParameters(ast, escope.default.analyze(ast, scopeOptions));
		});
